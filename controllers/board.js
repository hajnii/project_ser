...
exports.myWrite = async (req, res, next) => {
  ...
  let limit = req.query.limit;
  let query = `
              select 'board' as type,board_id,null as question_id,title,b.category,content,b.created_at,u.nickname,u.email,(select count(*) from p_boardview where board_id = b.board_id) as view_cnt,(select count(*) from p_comment where board_id = b.board_id) as com_cnt,b.starttime,b.endtime
              from p_board b join p_user u on b.user_id = u.id
              union
              select 'question' as type, null as board_id,question_id as board_id,title,q.category,content,q.created_at, u.nickname,u.email,(select count(*) from p_boardview where question_id =q. question_id) as view_cnt,(select count(*) from p_comment where question_id = q. question_id) as com_cnt,null as starttime,null asendtime
              from p_question q join p_user u on q.user_id = u.id
              order by created_at desc limit ${offset},${limit}
            `;
  console.log(query);
  try {

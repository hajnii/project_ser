...
exports.getCommentlist = async (req, res, next) => {
  ...
  let query = `select c.* , u.nickname from p_comment as c join p_user as u on c.user_id = u.id where board_id = ${board_id} order by created_at desc limit ${offset}, ${limit}`
  console.log(query);
